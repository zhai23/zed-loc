...
用法: .\build_windows.ps1 [-Backend <vulkan|opengl|both>] [-Clean] [-Help]
...
示例:
  .\build_windows.ps1                    # 构建 Vulkan 版本
  .\build_windows.ps1 -Backend opengl    # 构建 OpenGL 版本
  .\build_windows.ps1 -Backend both      # 构建两个版本
  .\build_windows.ps1 -Clean             # 清理构建
"@
    exit 0
}
...
function Main {
    ...
    if (-not (Test-Command "git")) {
        Write-Error "Git 未找到，请安装 Git"
        exit 1
    }
    Write-Success "Git 已安装: $(git --version 2>&1)"

    if (-not (Test-Command "cargo")) {
        Write-Error "Rust/Cargo 未找到，请从 https://rustup.rs/ 安装"
        exit 1
    }
